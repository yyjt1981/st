...
$pv->Fields['helpkindid'] = $kindid;

$pv->Fields['kindname']=$kindname;
	
$pv->SetTemplet(SLINETEMPLATE ."/".$cfg_df_style ."/" ."help/" ."help_show.htm");
...
function getHelpkind($aid)
{
   global $dsql;
   $kindname=array();
   $sql="select a.aid as kindid,a.kindname from #@__help_kind a inner join #@__help b on a.aid=b.kindid where a.webid=0 and b.aid=$aid";
   $row=$dsql->GetOne($sql);
   if(is_array($row))
   {
