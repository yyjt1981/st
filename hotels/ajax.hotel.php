...
if($dopost == 'gethotelroom_show')
{
	...
	$hotelinfo = getHotelInfo($hotelid);
	...
	foreach($arr as $row)
	{
		...
		$tablebody.='<tr>
				...
				<td width="100" align="center" valign="middle">'.getComputer($row['computer']).'</td>
				<td width="100" align="center" valign="middle">'.getBreakfirst($row['breakfirst']).'</td>
                        ...
                        <div class="control_con">
                          ...
                          <table width="918" border="0" cellspacing="0" cellpadding="0">
                            ...
                            <tr>
                              ...
                              <td>早餐：'.getBreakfirst($row['breakfirst']).'</td>
                              <td>窗户：'.$row['roomwindow'].'</td>
                            </tr>
                            <tr>
...
function getRoomPriceArr($year,$month,$roomid)
{
    global $dsql;
       ...
       $sql = "select * from #@__hotel_room_price where suitid='$roomid' and day >= '$nowtime'";
       $arr = $dsql->getAll($sql);
       $hotel = array();
       foreach($arr as $row)
       {
