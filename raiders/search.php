...
if(!empty($dest_id) && !empty($attrid))
{
    ...
    $sql = "select a.* from #@__article a left join {$left_table} on (b.classid='$attrid' and a.id=b.aid  and b.typeid=4 and b.destid='$dest_id') {$where} order by b.isding desc,b.displayorder asc,a.addtime desc";
}
else
{
    $left_table = '#@__kindorderlist b';
    $sql = "select a.* from #@__article a left join {$left_table} on (b.classid='$dest_id' and a.id=b.aid  and b.typeid=4 ) {$where} order by b.isding desc,b.displayorder asc,a.addtime desc";
}
...
$seoarr['pkname'] = getParkind($dest_id);
...
//注意以下这两句与伪静态规则有关系,不能写反了.
  ...
  $pv->SetTemplet(SLINETEMPLATE ."/".$cfg_df_style ."/" ."raiders/" ."article_search.htm");
