...
{
    global $dsql;
    $attlist = "row|5,kindid|,flag|,type|,row|6,readad|1,limit|0,destid|,pid|";
	...
	{
		 $tablename=array("1"=>"#@__line_kindlist","2"=>"#@__hotel_kindlist","3"=>"#@__car_kindlist","4"=>"#@__article_kindlist","5" =>"#@__spot_kindlist","6"=>"#@__photo_kindlist");
	...
	if($type=='gather')
	{
		...
		{
		  $sql="select id as kindid,kindname,pinyin,litpic from #@__destinations  where pid=$kindid order by displayorder asc limit 0,{$row}"; 	
		}
		else
		{ 
		   $sql="select id as kindid,kindname,pinyin,litpic from #@__destinations  where id=$kindid order by displayorder asc limit 0,{$row}"; 
    ...
    for($i=0;isset($ids[$i]);$i++)
    {
		...
		$pv = new View($typeid);
        $pv->Fields['leftad'] = getMiddleAd('indexleftad',$i);
