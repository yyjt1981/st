...
function lib_gethelplist(&$ctag,&$refObj)
{
	...
	{  
	   if(isset($refObj->Fields['sonid'])||!empty($sonid))
	   {
			...
			$sql="select aid,title from #@__help where webid=0 and   kindid={$sonid} $str order by displayorder asc,addtime desc limit 0,{$row}";
	...
	{
		$sql="select aid,title from #@__help where webid=0 order by displayorder asc,addtime desc limit 0,{$row}";
    }
    ...
    {
        $GLOBALS['autoindex']++;
		$row['url']=$GLOBALS['cfg_cmsurl']."/help/show_{$row['aid']}.html";
