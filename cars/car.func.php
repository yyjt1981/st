...
{
	global $dsql;
	$sql = "select a.* from #@__car a where a.aid = '$aid' and a.webid = 0";
	$row =$dsql->GetOne($sql);
	return $row;
}
/**
...
//获取当月价格
function getPrice($aid,$webid)
{
	global $dsql;
	$out='电询';
	$currentmonth=GetMonthHandle();
	$sql="select {$currentmonth} as price from #@__car_month where carid=$aid and webid=$webid";
	$row=$dsql->GetOne($sql);
	if(is_array($row))
	{
	  $out=(!empty($row['price'])) ? "￥".$row['price'] : "电询";
	}
	return $out;
...
{
	$arr=array('startplaceid','carkindid','brandid','priceid','seatnum','displayorder','attrid');
	$url="/cars/";
	$table="#@__car_attr";
