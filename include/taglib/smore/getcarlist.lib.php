...
function lib_getcarlist(&$ctag,&$refObj)
{
	...
	if($type=='mdd') 
	{
			...
			$sqlstr="select distinct a.id, a.*,b.minprice  from #@__car a left join(select min(price) as minprice,carid from #@__car_suit group by carid) as b on a.id=b.carid left join #@__kindorderlist as c on (c.classid=$pid and a.id=c.aid  and c.typeid=3)  where FIND_IN_SET($pid,kindlist) and a.webid=0 group by a.carname order by  c.isding desc,case when c.displayorder is null then 9999 end,c.displayorder asc limit $limit,$row";
	...
	else
	{
	  switch($flag)
	  {
		  case 'hot'://热门?暂时以istop排序？ 查询a结果可以优化
			  $sqlstr="select a.*,b.minprice,c.kindname as brand,d.kindname as kind   from #@__car a left join (select min(price) as minprice,carid from #@__car_suit group by carid) as b on a.id=b.carid  left join #@__car_brand c on c.aid=a.carbrandid and c.webid=a.webid left join #@__car_kind d on d.aid=a.carkindid and a.webid=d.webid where a.ishidden=0 and a.webid=0  order by a.displayorder asc, a.shownum desc,a.addtime desc limit $limit,$row";
			  //echo $sqlstr;
			 
	  
			  
		  break;
		  case 'recommend'://排序顺序：置顶》排序》最新更新

			  $sqlstr="select a.*,b.minprice,c.kindname as brand,d.kindname as kind   from #@__car a left join (select min(price) as minprice,carid from #@__car_suit group by carid) as b on a.id=b.carid left join #@__car_brand c on c.aid=a.carbrandid and a.webid=c.webid left join #@__car_kind d on d.aid=a.carkindid and a.webid=d.webid where a.ishidden=0 and a.webid=0 order by a.isding desc,a.isjian desc,a.displayorder asc limit $limit,$row";
		  ...
		  case 'listofkind':
			  ...
			  $sqlstr="select a.*,b.minprice   from #@__car a left join (select min(price) as minprice,carid from #@__car_suit group by carid) as b on a.id=b.carid  where FIND_IN_SET($pid,kindlist) and a.webid=0 order by  a.numberid asc,a.isrecommend desc,a.addtime desc limit $limit,$row";
		  ...
		  case 'relative':
			 ...
			 if($tagword=='') return '';
			  ...
			  $sqlstr="select a.*,b.minprice,c.kindname as brand,d.kindname as kind   from #@__car a left join (select min(price) as minprice,carid from #@__car_suit group by carid) as b on a.id=b.carid left join #@__car_brand c on c.aid=a.carbrandid and c.webid=a.webid left join #@__car_kind d on d.aid=a.carkindid and d.webid=a.webid  where  {$where} and a.ishidden=0 and a.webid=0  order by a.istop desc, a.numberid asc, a.addtime desc limit $limit,$row";
	...
	for($i=0;$i < $totalRow;$i++)
    {
		 ...
		 {
			 $tablerow['url'] = $GLOBALS['cfg_cmsurl']."/cars/show_{$tablerow['aid']}.html";;
