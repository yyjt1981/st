...
{
    global $dsql;
    $attlist="row|8,typeid|,groupname|,groupid|";
    FillAttsDefault($ctag->CAttribute->Items,$attlist);
    extract($ctag->CAttribute->Items, EXTR_SKIP);
    
	$attrtable=array('1'=>'#@__line_attr','2'=>'#@__hotel_attr','3'=>'#@__car_attr','4'=>'#@__article_attr','5'=>'#@__spot_attr','6'=>'#@__photo_attr','13'=>'#@__tuan_attr');
    $innertext = trim($ctag->GetInnertext());
    $revalue = '';
	$typeid = empty($typeid) ? $refObj->Fields['typeid'] : $typeid;
	$tablename=$attrtable[$typeid];
	if(!empty($groupname)) //通过组名获取
	{
	 $sql="select id from {$tablename} where  attrname='$groupname'";
	 $row=$dsql->GetOne($sql);
	 if(is_array($row))
	 $pid=$row['id'];
	}
	...
	if(empty($pid))return;
	$sql="select id,attrname from {$tablename} where pid='$pid' order by displayorder asc";
    $dsql->SetQuery($sql);
    $dsql->Execute();
    $ctp = new STTagParse();
