...
if($_POST['dopost']=='config')
{
    
    $serialNumber =$_POST['smsnumber'];//'0SDK-EMY-0130-JBQMR';
	$password = $_POST['smspwd'];
	...
	{	
	     ...
	     sendSMS($client);
		 $str='<?php $smsnumber=\''.$serialNumber.'\';'."\r\n".'$smspwd=\''.$password.'\';'."\r\n".'$smsbook=\''.$_POST['smsbook'].'\';'."\r\n".'$smsfpwd=\''.$_POST['smsfpwd'].'\';'."\r\n";  
...
function login($client)
{
	...
	if ($statusCode!=null && $statusCode=="0")
	{
		//登录成功，并且做保存 $sessionKey 的操作，用于以后相关操作的使用
		 return  'login_ok';//"登录成功, session key:".$client->getSessionKey()."<br/>";
	}else{
		//登录失败处理
		 return 'login_no';
	}
...
function sendSMS($client)
{
	 ...
	 */
	$statusCode = $client->sendSMS(array('13693443937'),"test2测试");
	//echo "处理状态码:".$statusCode;
}
