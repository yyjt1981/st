...
function lib_getslidenav(&$ctag,&$refObj)
{
    ...
    $revalue = '';
    $sql="select id,kindname as categoryname,linkurl as url,color from #@__plugin_leftnav where pid='{$kindid}' and isopen=1 order by displayorder asc limit {$limit},{$row}";
    $arr=$dsql->getAll($sql);
    for($i=0;isset($arr[$i]);$i++)
    {
        ...
        {
            $row['categoryname']="<a href=\"{$row['url']}\" target=\"_blank\" rel=\"nofollow\">{$row['categoryname']}</a>";
        }
...
function getCategoryChild2($kindid)
{
    ...
    for($i=0;isset($arr[$i]);$i++)
    {
        ...
        $outlist.=" <em><a href=\"{$row['linkurl']}\" target=\"_blank\"  rel=\"nofollow\">{$kindname}</a></em>";
    }
    return $outlist;
