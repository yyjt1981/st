...
class Mobile{
    ...
    var $tables=array(
        "lines"=>array('#@__line','line'),
        'hotels'=>array('#@__hotel','hotel'),
        'cars'=>array('#@__car','car'),
        ...
        $channel = array('lines','hotels','cars','raiders','spots','photos','visa','tuan');
            ...
            if(in_array($pathinfo[0],$channel)) //这里只判断栏目首页和显示页
            {
                if(empty($pathinfo[1])) //栏目首页
                {
                    ...
                    $mobileurl = $GLOBALS['cfg_basehost'].'/mobile/'.$item.'/';
                ...
                {
                    $mobileurl = $cfg_basehost.'/mobile/';//首页
                }
            }
            else
            {
    ...
    public  function getMobileShowUrl($type,$aid)
    {
        ...
        $url = $GLOBALS['cfg_basehost'].'/mobile/'.$this->tables[$type][1].'/show/'.$id;
