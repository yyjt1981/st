...
$templet="lines/line_search.htm";	//默认模板
...
{ 
  ...
  $fields="a.id,a.aid,a.webid,a.lineicon,a.linename as title,a.seotitle,a.sellpoint,a.linepic as litpic,a.storeprice,a.lineprice,a.linedate,a.transport,a.lineday,a.startcity,a.overcity,a.shownum,a.satisfyscore,a.bookcount,a.features,a.jifenbook,a.jifentprice,a.jifencomment,a.paytype,a.attrid,a.storeprice";
...
//查询sql   
$sql="select {$fields},a.linepic as litpic,b.isjian,b.isding,b.istejia from #@__line as a left join #@__kindorderlist b on(a.id=b.aid and b.typeid=1 and b.classid=$dest_id ) {$where}{$orderby} ";
...
//设置翻页参数(如果使用伪静态,则不能弄反了)
  ...
  $pv->SetTemplet(SLINETEMPLATE ."/".$cfg_df_style ."/" .$templet);
...
function getMinMaxprice($priceid)
{
  ...
  $arr=array();
   $arr['min']='';
   $arr['max']='';
  $sql="select lowerprice as min,highprice as max from #@__line_pricelist where aid=$priceid and webid=0";
  $row=$dsql->GetOne($sql);
