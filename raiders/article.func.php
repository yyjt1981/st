...
{
	global $dsql;
    $sql="select a.* from #@__article a where a.aid=$aid and a.webid='0'";
    $row=$dsql->GetOne($sql);
	return $row;
...
{ 
  ...
  {
    $sql="select a.kindname as shortname,b.seotitle,b.description from #@__destinations as a inner join #@__article_kindlist b on a.id=b.kindid where a.id={$childid} and isopen='1'";
  }
...
function getArticleMianbaoHtml($destid)
{
	 ...
	 {
		 $str.=' &gt; <a href="/raiders/'.$v['pinyin'].'/">'.$v['kindname'].'</a>';
	 }
	 return $str;
}
