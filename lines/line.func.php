...
function getJieShao_transfer($lineid)
{
    ...
    {
        $sql = "select * from #@__line_jieshao where lineid='$lineid'";
        $arr = $dsql->getAll($sql);
...
//获取最近报价
function getMonthPriceList($webid,$lineid,$year,$month)
{
	global $dsql;
	$thisyear=date('Y');
	$thismonth=date('m');
...
function getPiclistArr($row)
{
	...
	else if(empty($row['piclist'])&&!empty($row['linepic'])) //只上传封面的情况.
	{
		  ...
		  $biglist =' <li><img src="'.$row['linepic'].'" width="611" height="300"/></li>';  
	...
	else
	{
	   ...
	   {  
		  if(empty($value))
		  {
		   ...
		   $biglist .=' <li><img src="'.$temparr[0].'" alt="'.$temparr[1].'" width="611" height="300"/></li>';   
...
function getLineStartDate($row)
{
	global $dsql;
	$today=time();
    $montharr=getMonthPriceList($row['webid'],$row['aid'],date('Y',$today),date('m',$today));
  	$monthstr='';
	$monthli='';
	$curday=(int)date('d',$today);
    if(empty($montharr))
