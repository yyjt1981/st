...
function lib_flink(&$ctag,&$refObj)
{
	...
	if($showall=="all") //全部
	{
		$sql="select sitename,siteurl from #@__yqlj where webid='0'   order by addtime desc limit 0,{$row}";
	} 

	else //按栏目读取
	{
		$sql="select sitename,siteurl from #@__yqlj where webid='0' and locate($typeid,address) order by addtime desc limit 0,{$row}";
	}
    $dsql->SetQuery($sql);
    $dsql->Execute();
    $ctp = new STTagParse();
