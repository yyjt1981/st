...
//排序条件
   ...
   $sql="select a.*,b.isjian ,b.isding,b.istejia from #@__spot a left join #@__allorderlist b on (a.id=b.aid and b.typeid={$typeid} and a.webid=b.webid and b.classid='$dest_id') {$where} order by b.isding desc,b.displayorder asc  ";
...
$pv->SetSql($sql);
 ...
 $pv->SetParameter('attrid',$attrid);
  ...
  $pv->SetTemplet(SLINETEMPLATE ."/".$cfg_df_style ."/" ."spots/" ."spot_search.htm");
   ...
   /**
	...
	function getCurkind($id)
	{
		...
		if(is_array($pname))
		{
			...
			$str .= ' &raquo; <a href="' . $GLOBALS['cfg_cmsurl'] . '/spots/search.php?dest_id='.$id.'" rel="nofollow">' . $pname['kindname'] . '景点</a>';
		}
		else
		{
			$str .= '';
	...
	function getParKind($id)
	{
		...
		if(is_array($pname))
		{
			...
			$str = ' &raquo; <a href="' . $GLOBALS['cfg_cmsurl'] . '/spots/search.php?dest_id='.$pname['id'].'" rel="nofollow">' . $pname['kindname'] . '景点</a>';
		}
		else
		{
			$str = '';
