...
function lib_slidenavlist(&$ctag,&$refObj)
{
	...
	$sql="select id,linkurl,kindname,color from #@__plugin_leftnav where pid=0 and isopen=1 and webid=0 order by displayorder asc";
    ...
    for($k=0;isset($ids[$k]);$k++)
    {
      ...
      $pv->Fields['bottomad'] = getBottomAd($k);
...
//获取底部广告
function getBottomAd($whichone)
{
     global $dsql;
     $out='';
     $whichone = (int)$whichone + 1;

     $sql="select tagname,normalbody from #@__advertise where tagname = 'pluginad{$whichone}' order by displayorder asc ";
     $row=$dsql->GetOne($sql);
     if(is_array($row))
     {
         $out.= str_replace('padding-bottom:10px;','',$row['normalbody']);
     }
    return $out;
