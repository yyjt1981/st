...
{
    $time = time();
    $sql="select * from #@__line_suit_price where suitid='$suitid' and day >'$time'";
    //echo $sql;
    $dsql->SetQuery($sql);
    $dsql->Execute();
    $str='';
...
if($dopost == 'getpricebydate')
{
    ...
    $sql="select adultprice,childprice,oldprice from #@__line_suit_price where suitid='$suitid' and day ='$time'";
    $row = $dsql->GetOne($sql);
...
if($dopost == 'getoptionlist')
{
    ...
    $sql = "select * from #@__line_suit_price where suitid='$suitid' and day > '$time' limit 0,9";
    ...
    foreach($arr as $row)
    {
        ...
        $monthli.='<option value="'.$day.'" data-price="'.$adultprice.'" data-childprice="'.$childprice.'" data-oldprice="'.$oldprice.'">'.$text.'</option>';
