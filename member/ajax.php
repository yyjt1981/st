...
if($dopost == 'allorder')
{
	...
	$offset=($curpage-1)*$pagesize;
	$arr = $_model->getAll("memberid='$uid' {$w}","addtime desc","$offset,$pagesize");
	foreach($arr as $row)
	{
		$productname = getProductName($row['productautoid'],$row['typeid'],$row['productname']);//预订产品名称
		$totalprice = $row['dingnum'] * $row['price'] + $row['childnum'] * $row['childprice'];
	...
	$foot=' </table>';
    $out = !empty($out) ? $head.$out.$foot : $nocontent_msg;
	$totalnum = $_model->getCount("memberid='$uid'".$w);
	$totalpage=ceil($totalnum/$pagesize);;
	$pageinfo = setPageInfo($curpage,$totalpage);
	$data = array('list'=>$out,'pageinfo'=>$pageinfo);
	echo json_encode($data);
...
if($dopost == 'unpinlun')
{
	...
	$offset=($curpage-1)*$pagesize;
	if($typeid) $w = "and typeid='{$typeid}'";//添加订单类型判断
	$arr = $_model->getAll("memberid='$uid' and status=2 and ispinlun=0 {$w}","addtime desc","$offset,$pagesize");
	foreach($arr as $row)
	{
		$productname = getProductName($row['productautoid'],$row['typeid'],$row['productname']);//预订产品名称
		$totalprice = $row['dingnum'] * $row['price'] + $row['childnum'] * $row['childprice'];
...
if($dopost == 'wancheng')
{
    ...
    $offset=($curpage-1)*$pagesize;
    if($typeid) $w = "and typeid='{$typeid}'";//添加订单类型判断
    $arr = $_model->getAll("memberid='$uid' and status=2 {$w}","addtime desc","$offset,$pagesize");
    foreach($arr as $row)
    {
        $productname = getProductName($row['productautoid'],$row['typeid'],$row['productname']);//预订产品名称
        $totalprice = $row['dingnum'] * $row['price'] + $row['childnum'] * $row['childprice'];
