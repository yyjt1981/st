...
class Visit_Stats
{
	...
	function __construct($referrer, $php_self)
	{
	...
	public function Visit()
	{
		...
		//{
			if($this->check_keyword($keyword['searchengine'],$keyword['keywords'],$domain,$path))
			{
				$sql = "UPDATE #@__stats SET keynum=keynum+1 where searchengine='$keyword[searchengine]' and keywords='$keyword[keywords]'" . 
				       " and referer_domain='$domain' and referer_path='$path' and webid='0' and timemark='$mark'";
			}
			else
			{
				$sql = 'INSERT INTO #@__stats ( ' .
							'ip_address, visit_times, browser, system, language, ' .
							'referer_domain, referer_path, access_url, access_time, timemark, searchengine, keywords, keynum, webid' .
						') VALUES (' .
							"'$ip', '$visit_times', '$browser', '$os', '$lang', ".
							"'" . addslashes($domain) ."', '" . addslashes($path) ."', '" . addslashes($this->php_self) ."', '" . $time . "', '$mark', '" . 
							$keyword['searchengine'] . "', '" . $keyword['keywords'] . "', '1','0')";
			}
	...
	private function check_keyword($searchengine,$keywords,$referer_domain,$referer_path)
	{
		$Y = date("Y");	
		$M = date("n") < 10 ? "0" . date("n") : date("n");	
		$D = date("j") < 10 ? "0" . date("j") : date("j");	
		...
		$sql = "SELECT 1 FROM #@__stats where searchengine='$searchengine' and keywords='$keywords' and" . 
		       " referer_path='$referer_path' and referer_domain='$referer_domain' and webid='0' and timemark='$mark' limit 1";
