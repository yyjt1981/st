...

{
   ...
   $picwhere2=!empty($haspic)?" where (a.litpic is not null and a.litpic!='')":'';
    ...
    else if($type=='theme')
    {
        ...
        $sql="select a.* from #@__article a where FIND_IN_SET($themeid,a.themelist) $picwhere order by a.isding desc,a.displayorder asc limit {$limit},{$row}";

    }
    else if($type=='pinlun')
    {
        $sql = "select a.*,b.memberid,b.content as plcontent,b.addtime as pltime from #@__article a inner join #@__comment b on (a.id = b.articleid) group by a.articlename order by b.addtime desc limit {$limit},{$row}";
    }

	else if($flag=='specical')
	{
	   
	   $sql="select {$basefield} from #@__article a where webid is not null and isindex =1  $picwhere   order by  displayorder asc,addtime desc limit {$limit},{$row}";
	}
	else if($flag=='recommend')
	{
	   $sql="select {$basefield},b.isjian,b.isding as isding,b.displayorder from #@__article a left join #@__allorderlist b on (a.id=b.aid and b.typeid=4) $picwhere2   order by isding desc,b.isjian desc,case when b.displayorder is null then 9999 end,b.displayorder asc limit {$limit},{$row}";
	...
	else if($flag=='kindrecommend')
	{
	    ...
	    $sql="select {$basefield} from #@__article a where a.isjian=1 and FIND_IN_SET($kid,a.kindlist) $picwhere order by a.displayorder asc, a.addtime desc limit {$limit},{$row}";
	}
	else if($flag=='isindex')
	{
	   $sql="select {$basefield} from #@__article a where a.isindex=1 $picwhere  order by a.displayorder asc, a.addtime desc limit {$limit},{$row}";
	}
	else if($flag=='new')
	{
	   $sql="select {$basefield} from #@__article a $picwhere2  order by a.addtime desc limit {$limit},{$row}";
	  
	}
	else if($flag=='hot')
	{
	   $sql="select {$basefield} from #@__article a where a.webid IS NOT NULL $picwhere order by a.shownum desc,a.addtime asc limit {$limit},{$row}";
	}
	else if($flag=='photo') //幻灯显示
	{
	   	$sql="select {$basefield} from #@__article a where webid IS NOT NULL and a.litpic !='' $picwhere order by a.addtime desc limit {$limit},{$row}";
	...
	else if($flag=='jieban')
	{
		...
		$sql="select {$basefield},b.isding,b.displayorder,b.isjian from #@__article a left join #@__attrorderlist b on a.aid=b.aid and a.webid=b.webid where FIND_IN_SET($jiebanid,a.attrid) $picwhere order by b.isding desc,b.isjian desc, b.displayorder asc, a.addtime desc,a.modtime desc limit {$limit},{$row}";
	...
	else if($flag=='relative')
	{
	    ...
	    $sql="select a.* from #@__article a where  {$where} order by a.isding desc,a.displayorder asc,a.addtime desc limit {$limit},{$row} ";
	...
	else if($flag=='attr')
	{
		...
		$sql="select a.* from #@__article a where FIND_IN_SET($attrid,a.attrid) order by a.addtime desc limit {$limit},{$row}";
	}
	else if($flag=='byattr') //通过属性id调用文章
	{
		if(!empty($attrid))
		{
	       ...
	       $sql="select a.* from #@__article a left join #@__attrorderlist b on a.aid=b.aid and a.webid=b.webid where $where $picwhere order by b.isding desc,b.isjian desc, b.displayorder asc, a.addtime desc,a.modtime desc limit {$limit},{$row}";
		}
		else if(!empty($attrname))
		 {
			 ...
			 if(empty($temp_one))
			    ...
			    $sql="select a.* from #@__article a  where FIND_IN_SET({$temp_one['id']},a.attrid) order by order by a.modtime desc limit {$limit},{$row}";
		 }
    ...
    while($row = $dsql->GetArray())
    {
		...
		$newcomment=$dsql->GetOne("select * from #@__comment where typeid=4 and articleid={$row['id']} and pid=0 order by addtime desc");
