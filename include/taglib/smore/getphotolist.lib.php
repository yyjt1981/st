   ...
   //加目的地页面显示条件
	...
	{  
	   if(isset($refObj->Fields['kindid']))
	   {
			...
			$sql="select {$basefield} from #@__photo as a left join #@__kindorderlist as c on (c.classid=$sonid and a.id=c.aid and c.typeid=6) where  FIND_IN_SET($sonid,a.kindlist) order by c.isding desc,case when c.displayorder is null then 9999 end,c.displayorder asc limit {$limit},{$number}";
	...
	{  
	   ...
	   {
	        $sonid=$refObj->Fields['kindid'];
			$sql="select {$basefield} from #@__photo a where FIND_IN_SET($sonid,kindlist)  order by shownum desc limit 0,{$row}";	
	   }
	   else return '';
	} 
	//同目的地相册
	...
	{  
		...
		$sql="select {$basefield} from #@__photo a where {$where} order by displayorder desc limit 0,{$row}";
	...
	{  
	   if(isset($refObj->Fields['sonid']))
	   {
			...
			$sql="select aid,photoname,seoname,headimg,imginfo,title as littitle,alt,author,shownum,webid,headimgid from #@__photo  where webid!='' {$destwhere} and  isding=1 and photokind={$sonid} order by orderid  asc limit 0,{$row}";
	...
	//推荐相册
	else if($flag=='recommend')
	 {
		 ...
		 $sql="select {$basefield} ,b.isjian,b.isding ,b.displayorder from #@__photo a left join #@__allorderlist b on(a.id=b.aid and b.typeid=6) order by isding desc,case when b.displayorder is null then 9999 end,b.displayorder asc limit {$limit},{$row}";
		 }
		
		//置顶相册
		else if($flag=='isding')
		{
		   $sql="select {$basefield} from #@__photo where webid=$sys_webid  and isding=1 order by displayorder  asc limit {$limit},{$row}";
		}
		else if($flag=='roll')
		{
			//首页相册显示方式
		  ...
		  {
			  $sql="select {$basefield} from #@__photo where webid=$sys_webid order by modtime desc";
		   }
